---
title: "Human Genome Analysis Lab 7 : Linking 23andME SNPs to the NHGRI-EBI Catalog of published genome-wide association studies"
output:
  html_document:
    toc: true
    toc_depth: 4
...
The GWAS catalog is getting very large (~250 MB) and difficult to download with intermittent or not fast internet connections. I have downloaded the table and put it in the data folder. FOR UNITY USERS it is also available in Moodle (Github doesn't allow files greater than 100 MB).
...
ggplot(filter(trait_snp_proportion_study_count , count_of_studies > 1 & proportion_of_snps_for_trait > 70), aes(x = reorder(DISEASE.TRAIT, proportion_of_snps_for_trait, sum), y = proportion_of_snps_for_trait, fill = mean_risk_allele_freq)) +
 geom_col() +
 coord_flip() +
 theme_bw() + 
...
Use str() to show columns and data types. Which column in `mySNPs_gwas_table` shows the allele frequency in a population? What does it mean if the allele frequency is high in terms of linkage to disease whose trait is only expressed in 1% of the population.

### Exercise #2

Modify the below code to select for Type 2 Diabetes risk alleles (put the trait of interest in lower case)
